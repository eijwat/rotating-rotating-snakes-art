...
# アニメーション
ROTATION_SPEED = 80.0    # 1回転にかかる秒数（大きいほどゆっくり＝判別困難）
ROTATION_SECS = 5.0     # ディスクを回転させる時間（秒）
PAUSE_SECS = 1.0         # 回転後の静止時間（秒）
...
def _hash_noise(x, y, seed=0):
    ...
    def _h(ix, iy):
        n = (ix * 1619 + iy * 31337 + seed) & 0x7FFFFFFF
        n = (n >> 13) ^ n
...
def _fbm(x, y, octaves=5, seed=0):
    """Fractional Brownian Motion（多重スケールノイズ）"""
    v = np.zeros_like(x, dtype=np.float64)
    amp, freq = 1.0, 1.0
    for i in range(octaves):
        v += amp * _hash_noise(x * freq, y * freq, seed + i * 997)
        amp *= 0.5
        freq *= 2.0
    return v


def apply_fire(base_img, cx, cy, radius, t):
    """回転ディスクの全周を炎で囲み、炎は上方向に立ち上る

    FBM ノイズで揺らぐ炎テクスチャをディスク縁の全周に生成。
    全方向に炎が出るが、上方向にやや長く伸びる。
    ノイズは画面上方向（y減少方向）に流れ、炎が上昇する表現となる。
    base_img は RGBA の PIL Image。
    """
    bw, bh = base_img.size
    margin = 55  # 炎の広がり（px）

    # 計算範囲（ディスク周辺、全方向に余裕）
    rx0 = max(0, cx - radius - margin)
    ry0 = max(0, cy - radius - margin - 15)
    rx1 = min(bw, cx + radius + margin)
    ry1 = min(bh, cy + radius + margin)
    rw, rh = rx1 - rx0, ry1 - ry0

    # 座標グリッド
    yy, xx = np.mgrid[ry0:ry1, rx0:rx1].astype(np.float64)
    dx = xx - cx
    dy = yy - cy
    ...
    # ノイズ座標
    # 各ピクセルのノイズを「ディスク中心から外向き」＋「上方向に流す」で計算
    # → 炎が縁から外に向かって立ち上がるように見える
    ns = 0.07
    # 放射方向の座標（ディスクの外向き）
    radial_x = xx * ns
    radial_y = yy * ns
    # 上方向への流れ（y が小さい方が上 → ny を時間とともに減少）
    flow_y = -t * 5.0

    nx = radial_x + t * 0.8
    ny = radial_y + flow_y

    n1 = _fbm(nx, ny, 5, seed=0)
    n2 = _fbm(nx * 2.0 + 50, ny * 2.0 + 50, 4, seed=333)
    n3 = _fbm(nx * 0.4 + 200, ny * 0.4 + flow_y * 0.3, 3, seed=777)
    noise = n1 * 0.4 + n2 * 0.35 + n3 * 0.25

    # ディスク縁からの正規化距離
    edge_dist = (dist - radius) / margin

    # 全周に炎を出す（上方向にやや長い）
    # cos_up: 上=+1, 下=-1
    cos_up = -np.sin(angle)
    # 上方向は1.4倍、下方向は0.7倍（全周に出るが上がやや長い）
    height_mod = np.clip(0.85 + 0.4 * cos_up, 0.6, 1.4)

    # 基本減衰（縁から離れるほど弱く）
    falloff = np.clip(1.0 - edge_dist * 0.8, 0, 1) ** 0.6

    # ディスク内縁のグロー
    inner = np.where(edge_dist < 0,
                     np.clip(1.0 + edge_dist * 6, 0, 1) * 0.25, 0.0)

    # 合成強度（下半分の抑制なし → 全周）
    intensity = np.clip((falloff * height_mod + inner) * noise, 0, 1)

    # マスク
    mask = ((dist > radius - 8) & (dist < radius + margin)
            & (intensity > 0.02))

    # カラーマップ: 暗赤 → 赤 → オレンジ → 黄白
    i = intensity
    r_ch = np.clip(100 + 155 * np.sqrt(i), 0, 255)
    g_ch = np.clip(20 + 160 * i * i, 0, 180)
    b_ch = np.clip(50 * i * i * i, 0, 60)
    alpha = np.clip(i * 230, 0, 220)
    alpha[~mask] = 0

    fire = np.stack([r_ch, g_ch, b_ch, alpha], axis=-1).astype(np.uint8)
    fire_img = Image.fromarray(fire, "RGBA")
    fire_img = fire_img.filter(ImageFilter.GaussianBlur(radius=2))

    # フルサイズレイヤーに配置
    fire_full = Image.new("RGBA", (bw, bh), (0, 0, 0, 0))
    fire_full.paste(fire_img, (rx0, ry0))

    # ソフトグロー（広がりのある発光）
    glow = fire_full.copy().filter(ImageFilter.GaussianBlur(radius=6))

    frame = base_img.copy()
    frame = Image.alpha_composite(frame, glow)
    frame = Image.alpha_composite(frame, fire_full)
    return frame


# ─── フレーム生成・エンコード ─────────────────────────

def generate_mp4(img_rgba, img_rgb, disk_images, plan, out_path):
    ...
    for cycle, disk_idx in enumerate(plan):
        ...
        fire_label = " 🔥" if FIRE_EFFECT else ""
        print(f"  サイクル {cycle}: ディスク {disk_idx} "
              f"({cx},{cy}) {label}{fire_label}")

        # 回転フレーム
        for f in range(n_rot):
            t = f / MP4_FPS
            angle = direction * (t / ROTATION_SPEED) * 360.0
            ...
            if FIRE_EFFECT:
                # RGBA で炎を合成してから RGB に変換
                frame_rgba = frame.convert("RGBA")
                frame_rgba = apply_fire(frame_rgba, cx, cy,
                                        DISK_RADIUS, t)
...
def main():
    ...
    if not os.path.exists(IMAGE_PATH):
        ...
        print(f"  → KitaokaPosi_640.jpg をこのスクリプトと同じフォルダに"
              f"配置してください")
        sys.exit(1)
    ...
    print(f"  炎エフェクト: {'ON' if FIRE_EFFECT else 'OFF'}")
    print()

    # === MP4 ===
    mp4_path = "rotating_snakes.mp4"
    print(f"[1/2] MP4 生成中 ({MP4_FPS}fps, {img_rgb.size[0]}x"
          f"{img_rgb.size[1]})...")
    if generate_mp4(img, img_rgb, disk_images, plan, mp4_path):
